...
class Dictionary:
    def __init__(self, public_path: str, user_data_path: str):
            ...
            """
            logger.debug(f"Initializing dictionary with public_path: '{public_path}' and user_data_path: '{user_data_path}'")
            ...
            if filename.endswith(".json"):
                ...
                try:
                    with open(file_path, 'r', encoding='utf-8') as f:
                        data = json.load(f)
                        logger.debug(f"Loading dictionary '{data.get('name', 'unknown')}' from {filename}")
